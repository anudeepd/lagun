...
async def test_private_connections_are_visible_only_to_their_owner(
    keep_event_loop_awake,
):
    await session_store.init_db()
    alice = await session_store.create_session(
        SessionCreate(name="Alice", username="db"), "alice"
    )
    await session_store.create_session(SessionCreate(name="Bob", username="db"), "bob")

    assert [s.name for s in await session_store.list_sessions_for_user("alice")] == [
        "Alice"
    ]
...
async def test_shared_connection_can_be_hidden_by_one_user_only(
    tmp_path, monkeypatch, keep_event_loop_awake
):
    await session_store.init_db()
...
""")
    ...
    assert [s.name for s in await session_store.list_sessions_for_user("bob")] == [
        "Shared DB"
    ]
...
async def test_managed_connection_can_select_databases_from_config(
    tmp_path, monkeypatch, keep_event_loop_awake
):
    await session_store.init_db()
    config = tmp_path / "connections.yaml"
