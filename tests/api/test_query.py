...
async def test_script_query_executes_ordered_writes_in_one_transaction(
    client, session_id, test_db
):
    r = await client.post(
        f"/api/v1/sessions/{session_id}/query/script",
        json={
...
async def test_script_query_validate_rejects_update_without_where(
    client, session_id, test_db
):
    r = await client.post(
        f"/api/v1/sessions/{session_id}/query/script/validate",
        json={
...
async def test_query_nonexistent_session(client):
    r = await client.post(
        "/api/v1/sessions/does-not-exist/query", json={"sql": "SELECT 1"}
    )
...
async def test_cell_update(client, session_id, test_db):
    ...
    r2 = await client.post(
        f"/api/v1/sessions/{session_id}/cell-update",
        json={
            "database": test_db,
            "table": "users",
            "column": "age",
...
async def test_row_insert(client, session_id, test_db):
    r = await client.post(
        f"/api/v1/sessions/{session_id}/row-insert",
        json={
            "database": test_db,
            "table": "users",
    ...
    assert (
        data["sql_executed"]
        == f"INSERT INTO `{test_db}`.`users` (`name`, `age`) VALUES ('Dave', 40)"
    )
...
async def test_row_insert_empty_values_uses_defaults(client, session_id, test_db):
    ...
    assert data["affected_rows"] == 1
    assert (
        data["sql_executed"] == f"INSERT INTO `{test_db}`.`default_only` () VALUES ()"
    )
...
async def test_row_insert_error_returns_attempted_sql(client, session_id, test_db):
    ...
    assert data["error"]
    assert (
        data["sql_executed"] == f"INSERT INTO `{test_db}`.`users` (`age`) VALUES (41)"
    )
...
async def test_row_delete(client, session_id, test_db):
    ...
    r2 = await client.request(
        "DELETE",
        f"/api/v1/sessions/{session_id}/rows",
        json={
            "database": test_db,
            "table": "users",
    ...
    assert data["affected_rows"] == 1
    assert (
        data["sql_executed"] == f"DELETE FROM `{test_db}`.`users` WHERE `id` = {ids[0]}"
    )
...
async def test_row_delete_without_primary_key_uses_all_columns(
    client, session_id, test_db
):
    await client.post(
        f"/api/v1/sessions/{session_id}/query",
...
async def test_row_delete_large_bigint_primary_key_round_trips_as_string(
    client, session_id, test_db
):
...
async def test_script_query_kill_returns_error_when_no_active_script(
    client, session_id, test_db
):
    r = await client.delete(
        f"/api/v1/sessions/{session_id}/query/script/nonexistent-id"
    )
    assert r.status_code == 200
    data = r.json()
    assert data["ok"] is False
        ...
        async def run_first():
            return await client.post(
                ...
                json={
                    ...
                    "database": test_db,
                    "sql": ";".join(
                        f"INSERT INTO users (name, age) VALUES ('conc-{i}', {i})"
                        for i in range(2999)
                    ),
                },
            )
