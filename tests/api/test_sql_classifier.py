...
FIXTURE_PATH = Path(__file__).resolve().parent.parent / "fixtures" / "sql_classifier.json"
...
def test_allowed_statements_pass_validation(case):
    statements = _split_sql_script(case["sql"])
...
def test_rejected_statements_fail_validation(case):
    statements = _split_sql_script(case["sql"])
    result = _validate_script_statements(statements)
    assert result.ok is False, f"Expected rejection for {case['label']} but validation passed"
...
def test_lock_wait_timeout_detection_by_errno():
    assert _is_lock_wait_timeout(Exception(1205, "Lock wait timeout exceeded; try restarting transaction"))


def test_lock_wait_timeout_detection_by_message():
    assert _is_lock_wait_timeout(Exception("Lock wait timeout exceeded; try restarting transaction"))
