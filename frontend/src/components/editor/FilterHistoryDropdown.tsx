 ...
 */
export default function FilterHistoryDropdown({ history, disabled, onSelect }: FilterHistoryDropdownProps) {
  ...
  useLayoutEffect(() => {
    ...
    const clamp = () => {
      ...
      if (!rect) return
      const width = menuRef.current?.offsetWidth ?? 320
    ...
    <AnimatePresence>
      {open && (
        <m.div
          ...
          className="fixed z-popover max-h-72 max-w-[calc(100vw-16px)] min-w-[280px] overflow-y-auto rounded-md border border-surface-700 bg-surface-900 py-1 shadow-2xl"
        ...
        >
          {history.map((filter, i) => (
            <button
              key={`${filter}-${i}`}
              role="menuitem"
              className="block w-full truncate px-3 py-1.5 text-left font-mono text-xs text-slate-300 hover:bg-surface-800"
              title={filter}
              onClick={() => {
                setOpen(false)
                onSelect(filter)
